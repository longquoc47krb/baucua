...
function BettingZone({ item }: BettingZoneProps) {
    ...
    const handleStep = () => {
        const arr = [10000, 20000, 50000, 100000, 200000, 500000, 1000000, 10000000];
        const indexOfStep = arr.indexOf(step);
        if (indexOfStep !== -1) {
            // Check if there is a next step in the array
            if (indexOfStep < arr.length - 1) {
                // Update the step to the next one
                const nextStep = arr[indexOfStep + 1];
                setStep(nextStep);
            } else {
    ...
    return (
        <div className='relative p-4 bg-[#faefdb] border-2 border-[#eabd68] w-[280px] flex items-center flex-col min-h-[40vh]'>
